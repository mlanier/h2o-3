...
def h2oimport_sql_table():
    ...
    try:
        ...
        assert citi_sql.nrow == 2e4, "h2o.import_sql_table() command not working."
        assert citi_sql.ncol == 2, "h2o.import_sql_table() command not working."

        sql_select = h2o.import_sql_select(conn_url, "SELECT starttime FROM citibike20k", username, password)
        assert sql_select.nrow == 2e4, "h2o.import_sql_table() command not working."
        assert sql_select.ncol == 1, "h2o.import_sql_table() command not working."
    except Exception as e:  # some errors are okay like version mismatch
        assert False, "h2o.import_sql_table() command not working."
